...
export default (io) => {
  ...
  router.post('/verify-child-code', async (req, res) => {
    ...
    try {
      ...
      const newChildFromDB = insertChildResult.rows[0]; // Get the newly generated child ID

      const newChildDataForFrontend = {
        id: newChildFromDB.id,
        name: newChildFromDB.name,
        status: newChildFromDB.status,
        location: newChildFromDB.current_zone || 'Unknown',
        lastupdated: newChildFromDB.last_seen, // Ensure it's in a compatible format for frontend
        coordinates: {
            lat: newChildFromDB.last_latitude || 0,
            lng: newChildFromDB.last_longitude || 0,
        },
      };

      // Emit to the specific parent's room
      io.to(parent_id).emit('newChildAdded', newChildDataForFrontend);
      console.log(`Emitted 'newChildAdded' to parent ${parent_id} for child ${newChildFromDB.id}`);

      res.status(200).json({
          message: "Code verified successfully and child registered.",
          parentId: parent_id,
          childName: child_name,
          childId: newChildFromDB.id // Include the new child's ID for the frontend
      });
    } catch (error) {
